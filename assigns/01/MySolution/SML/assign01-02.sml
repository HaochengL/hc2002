...
(xs: 'a xlist, i0: int): 'a = 
    case xs of 
        ...
        | xlist_cons(x1,xs) => if i0<=0 then x1 else if i0 > xlist_size(xs)-1 then raise XlistSubscript else  xlist_sub(xs, i0-1) 
        | xlist_snoc(xs,x1) => if i0<=0 then x1 else if i0 > xlist_size(xs)-1 then raise XlistSubscript else  xlist_sub(xs, i0-1) 
